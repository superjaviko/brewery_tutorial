...
import { StageName, quizData, Question } from "../data/quizData";
...
export default function StageDetail({ stage, onBack, onCompleteStage}: StageDetailProps) {
  ...
  const [acidLacticCount,setAcidLacticCount]= useState(0);
  ...
  const beerColor = `hsl(${45 + srm * 3}, 80%, 40%)`; // Simulate color scale
  ...
  const [grainSliderValue, setGrainSliderValue] = useState(2); // default to 0.9
...
const [isSparging, setIsSparging] = useState (false);
...
const handleStageComplete = () => {
   setShowQuiz(true); // Mostramos el quiz antes de pasar al siguiente stage
};

const handleQuizComplete = (score: number) => {
  setShowQuiz(false);
  onCompleteStage(stage.name, score); // Ahora sí avanzamos al siguiente stage
};
...
useEffect(() => {
  const interval = setInterval(() => {
    const newPoint = {
      ...
      temp: Math.floor(Math.random() * 3) + 20, // Rango: 22–24°C
    };
    setData((prevData) => {
      const lastTemp = prevData.length > 0 ? prevData[prevData.length - 1].temp : 23;
      const variation = Math.random() * 0.8 - 0.4; // -0.4 a +0.4
  ...
  useEffect(() => {
    // Reset everything when stage changes
  ...
  useEffect(() => {
    if (phLevel <= 5.6 && phLevel >= 5.2) {
      ...
      // Opcional: ocultar después de algunos segundos
  ...
  useEffect(() => {
    if (isMixing) {
      setProgress(0); // Reinicia progreso al comenzar
      const duration = 5000; // 5 segundos
      const intervalTime = 100; // cada cuánto se actualiza
      const increment = 100 / (duration / intervalTime); // cuánto sube cada tick
      ...
      const interval = setInterval(() => {
        setProgress((prev) => {
          ...
          if (next >= 100) {
            clearInterval(interval);
            setIsMixing(false);
            setIsMashing(true); // continúa al siguiente estado si lo deseas
            return 100;
          }
          return next;
        });
    ...
    if (allConnected && !coolingStarted) {
      ...
      const interval = setInterval(() => {
        ...
        setCoolingTemperature((prev) => {
          ...
          return prev - ((70 - 18) / 100); // Baja de 70 a 18 a lo largo del progreso
        });
      }, 100); // 10 segundos en total
    }
  ...
  useEffect(() => {
    if (spargingProgress >= 100) {
      setShowPourMessage(true); // Mostrar el mensaje cuando la barra termine
    }
  }, [spargingProgress]);
  ...
  const handleMashingDrop = (ingredient: string) => {
    ...
    if (ingredient === "acid lactic") {
      setAcidLacticCount((prev) => prev + 1);
      setPhLevel((prev) => Math.max(4.2, +(prev - 0.2).toFixed(1))); // Bajar de 0.2 en 0.2
    }
  };
              ...
              {isMilling ? (
                <>
                  ...
                  <div className="w-full mt-4 px-4">
                    ...
                    <p className="text-sm text-gray-600 mt-1 text-center"> Milling the malt grain...{progress}%</p>
                  </div>
                </>
              ) : (
                <p className="text-gray-600 text-center">
                  {grainDropped ? "✅ Milling complete!" : "🛠️ Drag the grain into the mill"}
                </p>
              )}
            </AnimatePresence>
...
{stage.name === "Mashing" && (
  ...
  />
  {grainDropped && hotWaterDropped && phLevel=== 5.6 && !isMixing && (
  <motion.button
...
{grainDropped && hotWaterDropped && !isMixing && (phLevel < 5.5 || phLevel > 5.7) && (
    <motion.p
    key="instruction"
...
<AnimatePresence mode="wait">
  ...
  <p className="mt-2 text-sm text-gray-700">{phLevel.toFixed(1)}</p>
</div>

  <div className="relative z-10 flex flex-col items-center">

    {/* ✅ Botón visiblemente interactivo */}
    <AnimatePresence>
  ...
  <div className="relative space-y-10 mt-20">
      ...
      <>
        ...
        {!grainDropped && (
          <motion.img
            ...
            dragConstraints={{ left: 0, right: 300, top: 0, bottom: 300 }}
            onDragEnd={(_event, info) => {
              if (info.point.x > 400) { // Ajusta el punto para soltar
                setGrainDropped(true);
                setShowDragMessage(false)
                setTimeout(() => setSpargingComplete(true), 500); // Añadir pequeño delay
              }
            }}
            initial={{ opacity: 0, scale: 0.5 }}
            animate={{ opacity: 1, scale: 1 }}
            ...
            const interval = setInterval(() => {
              setFlowProgress((prev) => {
                if (prev >= 100) {
                  ...
                  setShowDragMessage(true); // 👉 Mostrar mensaje de draggear grain
                }
                return Math.min(prev + 2, 100);
              });
            }, 100);
    ...
    <div className="flex justify-between items-center w-full px-8 mt-40">
      <div className="relative w-50 h-60 border rounded-xl bg-amber-100 flex opacity-80 items-center justify-center">
        <img src="/kettle.png" alt="Kettle" className="w-50 h-50 opacity-90" />
        {spargingStarted && (
          <motion.svg
            width="180"
            height="200"
  ...
  <div className="space-y-6">
        ...
        <>
          {!isMilling && (
            <motion.img
              ...
              onDragEnd={(_event, info) => {
                if (info.point.x > 200) {
                  setIsMilling(true); // now used for hopsDropped
                }
              }}
            />
          )}
          ...
          {!progress && (
            <motion.img
              ...
              onDragEnd={(_event, info) => {
                if (info.point.x > 200) {
                  setProgress(1); // now used for yeastDropped
                }
              }}
            />
          )}
    ...
    <div className="flex flex-col items-center">
      <div className="relative w-60 h-60 border rounded-xl bg-orange-200 flex items-center justify-center">
        ...
        {isMilling && <img src="/hops.svg" alt="Hops in" className="w-6 absolute top-6 left-8" />}
        {progress > 0 && <img src="/hop-yellow.png" alt="Hop-Yellow in" className="w-6 absolute bottom-4 right-8" />}
        {progress > 0 && <img src="/hop-orange.png" alt="Hop-Orange in" className="w-6 absolute bottom-6 right-4" />}
        <img src="/temperature-sensor.png" alt="Temperature sensor" className="absolute w-14">
        </img>
      </div>      
...
{boilingStarted && (
  <div className="mt-8 flex flex-col items-center">
    ...
    <p className="mt-2 text-sm text-gray-700">Boiling in progress... {progress}%</p>
  </div>
)}
      ...
      {grainDropped && (
        <div className="w-full mt-4 px-8">
          ...
          {progress === 100 && (
            <p className="text-green-600 font-medium text-center mt-2">✅ Boiling complete!</p>
          )}
        </div>
      )}
...
{stage.name === "Cooling" && (
  <div className="relative">
    ...
    {/* 🔼 FILA SUPERIOR: Dispositivos */}
    <div className="flex flex-col items-center mt-10 space-y-6">
      <p className="text-lg font-semibold text-gray-700 text-center -mt-5">
        Connect these elements to cool and transfer the wort to the Fermenter
      </p>
    </div>
    <div className="grid grid-cols-3 gap-10 justify-items-center items-center mt-18">
      ...
      <img src="/fermentator.png" alt="Fermentator" className="w-24" />
      <img src="/chiller.svg" alt="Chiller" className="w-20" />
    </div>

    {/* 🔶 SVG entre fermentator y chiller (con forma especial) */}
    ...
    {/* 🔵 SVG entre water tap y counterflow */}



    {/* 🟡 SVG entre kettle y counterflow */}
    ...
    {/* 🔵 Línea de agua saliendo del fermentador */}
    ...
    <div className="mt-8 space-y-4">
      {connections.map((conn, idx) => (
        ...
        >
          {conn.connected ? "✅" : "🔗"} Connect {conn.from} → {conn.to}
        </button>
      ))}
<AnimatePresence>
{coolingStarted && (
...
{stage.name === "Fermenting" && (
    ...
    <p className="text-sm font-semibold text-gray-700 text-center -mt-0">
      Drag the Yest to the Fermenter
    </p>
    {/* 📦 Fermentator + Chiller */}
    <div className="flex justify-center gap-30 mt-40 items-center relative">
      ...
      {/* 🧲 Zona de drop invisible encima del fermentador */}
      <div
        ref={dropZoneRef}
  ...
  />
    ...
    {fermentationStarted && (
      ...
      <div className="text-center mt-8">
        <p className="text-lg font-semibold">Fermentation Week: {day} / 7</p>
  ...
  <div className="w-full mt-30">
    <div className="flex justify-between items-end w-full">
      ...
      <div className="items-center">
        <img src="/fermentator.png" alt="Fermentator" className="w-120" />
        <span className="mt-2 text-sm text-gray-500">Fermentator</span>
      </div>
      ...
      {/* 🏭 Bottling machine */}
      <div className="flex flex-col items-center">
        <img src="/bottling-machine.svg" alt="Bottling" className="w-32 mb-4" />
       ...
       <div className="flex gap-2">
        {[...Array(3)].map((_, i) => (
         <motion.div
           ...
           initial={{ height: "0%" }}
           animate={bottlingStarted ? { height: "100%" } : { height: "0%" }}
           transition={{
      ...
      <div className="flex items-end gap-1">
        {[...Array(3)].map((_, i) => (
          <motion.img
            ...
            className="w-12 h-12"
            initial={{ y: -20, opacity: 0 }}
            animate={bottlingStarted ? { y: 0, opacity: 1 } : { y: -20, opacity: 0 }}
            transition={{
